  ...
  void create_library_project(const fs::path& root, const options& opts)
  {
    ...
    else
    {
...

      {
        ...
        if (opts.init_tests)
        {
          cmake_file.writeln("option(" + option_prefix + "_BUILD_TESTS \"Build tests\" ON)");
        }

        cmake_file.writeln("option(" + option_prefix + "_INSTALL \"Generate install rules\" ON)")
...

      {
        ...
        source_file.writeln("#include \"" + opts.namespace_name + "/" + opts.project_name + ".hxx\"").writeln("");
